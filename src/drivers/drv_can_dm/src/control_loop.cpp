...
DmHWLoop::DmHWLoop(std::shared_ptr<DmHW> hardware_interface)
    ...
    // 默认电机配置（可通过构造函数参数传入）
    std::vector<damiao::MotorConfig> motor_configs = {{.bus_name = "can0",
        .can_id = 0x01,
        .master_id = 0x11,
    ...
    if (pthread_setschedparam(loop_thread_.native_handle(), SCHED_FIFO, &sched) != 0) {
        std::cerr << "Failed to set threads priority (one possible reason could be that the user and the group "
...
void DmHWLoop::update() {
    ...
    if (cycle_time_error > cycle_time_error_threshold_) {
        std::cerr << "WARN: Cycle time exceeded error threshold by: " << cycle_time_error - cycle_time_error_threshold_
                << "s, cycle time: " << elapsed_time_.count() << "s, threshold: " << cycle_time_error_threshold_
                << "s\n";
    }
    // Input
