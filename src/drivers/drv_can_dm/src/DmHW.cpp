...
bool DmHW::init(const std::vector<MotorConfig> &motor_configs) {
  ...
  for (const auto &bus_motor_pair : bus_motors) {
    ...
    for (const auto &motor : motors) {
      bus_id2dm_data_[bus_name].insert(
          std::make_pair(motor.can_id, DmActData{.motorType = motor.motor_type,
...
void DmHW::sendMitCommand(const std::string &bus_name, uint16_t can_id, float p,
  ...
  std::cerr << "[DmHW] Error: Motor " << can_id << " not found on any bus."
            << std::endl;
}
...
void DmHW::setZeroPosition() {
  ...
  for (auto motor_port : motor_ports_) {
    ...
    for (const auto &motor_pair : motors) {
      ...
      std::cout << "[DmHW] Set zero position for motor ID: "
                << motor->GetCanId() << std::endl;
    }
  }
...
void DmHW::emergencyStop() {
  ...
  for (auto motor_port : motor_ports_) {
    ...
    for (const auto &motor_pair : motors) {
      ...
      std::cout << "[DmHW] Emergency stop for motor ID: " << motor->GetCanId()
                << " at position: " << current_pos << " rad" << std::endl;
    }
  }
...
void DmHW::startAutoRead(unsigned int period_ms) {
  ...
  read_thread_ = std::thread([this, period_ms]() {
    while (read_running_) {
      this->read(std::chrono::system_clock::now(),
                 std::chrono::duration<double>(period_ms/1000.0));
