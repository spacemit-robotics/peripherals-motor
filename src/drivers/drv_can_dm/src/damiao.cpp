...
Motor::Motor(DM_Motor_Type motor_type, Control_Mode ctrl_mode, uint16_t can_id,
             uint16_t master_id)
    : Motor_Type(motor_type), mode(ctrl_mode), Master_id(master_id),
      Can_id(can_id) {
...
Motor_Control::Motor_Control(std::string bus_name,
                             std::unordered_map<uint16_t, DmActData> *data_ptr)
    : data_ptr_(data_ptr) {
  for (auto it = data_ptr_->begin(); it != data_ptr_->end();
       ++it) { // 遍历该bus下的所有电机
    std::shared_ptr<Motor> motor =
        std::make_shared<Motor>(it->second.motorType, it->second.mode,
                                it->second.can_id, it->second.mst_id);
    addMotor(motor);
  }
  int thread_priority = 95;
  while (!socket_can_.open(bus_name,
                           boost::bind(&Motor_Control::canframeCallback, this,
...
float Motor_Control::read_motor_param(Motor &DM_Motor, uint8_t RID) {
  read_write_save =
      true; // 发送读参数命令，返回的数据和常规返回的不一样，需要单独处理
...
void Motor_Control::save_motor_param(Motor &DM_Motor) {
  ...
  usleep(10000);
  read_write_save =
      true; // 发送保存参数命令，返回的数据和常规返回的不一样，需要单独处理
...
void Motor_Control::write_motor_param(Motor &DM_Motor, uint8_t RID,
  ...
  read_write_save =
      true; // 发送写参数命令，返回的数据和常规返回的不一样，需要单独处理
...
void Motor_Control::control_mit(Motor &DM_Motor, float kp, float kd, float q,
                                float dq, float tau) {
  ...
  if (motors.find(id) == motors.end()) {
    std::cerr << "[Error] In control_mit,no motor with id "
              << DM_Motor.GetCanId() << " is registered." << std::endl;
...
void Motor_Control::control_pos_vel(Motor &DM_Motor, float pos, float vel) {
  uint16_t id = DM_Motor.GetCanId();
  if (motors.find(id) == motors.end()) {
    std::cerr << "[Error] In control_pos_vel,no motor with id "
              << DM_Motor.GetCanId() << " is registered." << std::endl;
    std::exit(-1); // 终止程序，返回非 0 表示错误
  }
  can_frame frame{};
...
void Motor_Control::control_vel(Motor &DM_Motor, float vel) {
  uint16_t id = DM_Motor.GetCanId();
  if (motors.find(id) == motors.end()) {
    std::cerr << "[Error] In control_vel,no motor with id "
              << DM_Motor.GetCanId() << " is registered." << std::endl;
    std::exit(-1); // 终止程序，返回非 0 表示错误
  }
  can_frame frame{};
...
void Motor_Control::receive_param(uint8_t *data) {
  ...
  if (motors.find(canID) == motors.end()) {
    std::cerr << "[Error] In receive_param,no motor with id " << canID
...
bool Motor_Control::switchControlMode(Motor &DM_Motor, Control_Mode_Code mode) {
  ...
  if (motors.find(DM_Motor.GetCanId()) == motors.end()) {
    std::cerr << "[Error] In switchControlMode,no motor with id "
              << DM_Motor.GetCanId() << " is registered." << std::endl;
    std::exit(-1); // 终止程序
    return false;
  }
...
bool Motor_Control::change_motor_param(Motor &DM_Motor, uint8_t RID,
  ...
  if (motors.find(DM_Motor.GetCanId()) == motors.end()) {
    std::cerr << "[Error] In change_motor_param,no motor with id "
              << DM_Motor.GetCanId() << " is registered." << std::endl;
    std::exit(-1); // 终止程序，返回非 0 表示错误
    return false;
  }
...
void Motor_Control::write() {
  ...
  for (const auto &m : *data_ptr_) { // 遍历该can接口下的所有电机
    ...
    if (motors.find(motor_id) == motors.end()) {
      std::cerr << "[Error] In write,no motor with id " << motor_id
                << " is registered." << std::endl;
      std::exit(-1); // 终止程序，返回非 0 表示错误
    }
    ...
    if (t < motion_time) {
      ...
      // 计算最大速度（匀速阶段的速度）
      double v_max =
          total_angle / (motion_time - 0.5 * accel_time - 0.5 * decel_time);
      ...
      if (t < accel_time) {
        ...
        // 位置是速度的积分
        target_pos = initial_positions[motor_id] +
                     v_max * accel_time * (s3 - 0.5 * s2 * s2);
      } else if (t < decel_start) {
        // 匀速阶段
        double accel_distance = v_max * accel_time * 0.5; // 加速阶段的距离
        target_vel = v_max;
        target_pos = initial_positions[motor_id] + accel_distance +
                     v_max * (t - accel_time);
      } else {
        ...
        target_pos = initial_positions[motor_id] + accel_distance +
                     const_distance + decel_distance;
      }

      // MIT 模式：位置 + 速度控制，使用传入的 kp 和 kd 参数
      control_mit(*it, motion_params_.kp, motion_params_.kd, target_pos,
                  target_vel, 0);
      ...
      if (!motion_completed) {
        ...
        target_vel = 0;
        control_mit(*it, motion_params_.kp, motion_params_.kd, target_pos, 0,
                    0);
...
void Motor_Control::read() {
  for (auto &m : *data_ptr_) {   // 遍历该can接口下的所有电机
    ...
    if (motors.find(motor_id) == motors.end()) {
      std::cerr << "[Error] In read,no motor with id " << motor_id
                << " is registered." << std::endl;
      std::exit(-1); // 终止程序，返回非 0 表示错误
    }
    auto &it = motors[motor_id];
    ...
    std::cerr << "pos: " << m.second.pos << " vel: " << m.second.vel
              << " effort: " << m.second.effort << std::endl;
  }
}
...
void Motor_Control::canframeCallback(const can_frame &frame) {
  ...
  for (const auto &frame_stamp : read_buffer_) {
    ...
    if (read_write_save == true &&
        motors.find(canID) !=
            motors.end()) { // 这是发送保存参数或者写参数或者读参数返回的数据
      if (frame.data[2] == 0x33 || frame.data[2] == 0x55 ||
          frame.data[2] ==
              0xAA) { // 发的是读参数或写参数命令，返回对应寄存器参数
        if (frame.data[2] == 0x33 || frame.data[2] == 0x55) {
      ...
      if (motors.find(frame.can_id) == motors.end()) {
        std::cerr << "[Debug] Received frame with ID " << std::hex
                  << frame.can_id << " but not found in map." << std::dec
                  << std::endl;
        return;
      }
