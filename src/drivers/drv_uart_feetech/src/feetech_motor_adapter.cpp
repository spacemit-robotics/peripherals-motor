...
static std::unordered_map<std::string, FeetechPack *>
    g_feetech_packs;  // 按设备路径管理
...
static int feetech_get_paras(struct motor_dev *dev, const void *address,
                                void *out_data, uint32_t data_len) {
    ...
    int ret = priv->pack->get_sms_sts().Read(
        priv->motor_id, mem_addr, reinterpret_cast<uint8_t *>(out_data),
        static_cast<uint8_t>(data_len));
    if (ret != static_cast<int>(data_len)) {
    std::cerr << "[Feetech] Read paras failed! ID: "
                << static_cast<int>(priv->motor_id)
...
static int feetech_set_paras(struct motor_dev *dev, const void *address,
    ...
    if (ret == -1) {
    std::cerr << "[Feetech] Write paras failed! ID: "
                << static_cast<int>(priv->motor_id)
                << ", Addr: " << static_cast<int>(mem_addr) << std::endl;
    return -1;
    }
...
static struct motor_dev *feetech_factory(void *args) {
    ...
    struct motor_args_uart *uart_args =
        reinterpret_cast<struct motor_args_uart *>(args);

    // 分配 motor_dev 结构
    struct motor_dev *dev =
        reinterpret_cast<struct motor_dev *>(malloc(sizeof(struct motor_dev)));
    if (!dev)
    return nullptr;
    ...
    std::cout << "[Feetech] Factory: motor_id="
            << static_cast<int>(priv->motor_id) << ", baud=" << actual_baud
            << std::endl;
