...
反馈帧低 5 位为状态码：`0` 无故障，`1` 过热，`2` 过流，`3` 过压，`4` 欠压，
`5` 编码器故障，`6` 抱闸电压过高，`7` 驱动故障。协议解码保留原始值；驱动向
`motor_state.err` 输出时将原厂控制器定义为非致命状态的欠压码 `4` 归一为 `0`，
避免瞬时欠压使整机控制退出。其他未定义的非零值仍保留给上层处理。
